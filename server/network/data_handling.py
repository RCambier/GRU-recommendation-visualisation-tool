...
class DataHandler(object):
    ...
    def _load_mapping(self,filename):
        mapping_dic = {}
        with open(self.dirname + 'data/' + filename, 'r') as f:
            for line in f:
                elem1, elem2 = map(int, line.split())
                mapping_dic[elem1] = elem2
        return mapping_dic


    def _load_titles(self,filename):
        data = pd.read_csv(self.dirname + 'data/' + filename, sep='::', header=None, index_col=False)
       # dic = {line[0]: str(line[1]) + "  " + str(line[2]) for line in data.as_matrix()}
        titles = {line[0]:{'title':line[1] ,'genre':line[2]}for line in data.as_matrix()}
        return titles

    # Features order: movie id, year, Action, Adventure, Animation, Children's, Comedy, Crime, Documentary, Drama, Fantasy, Film-Noir, Horror, Musical, Mystery, Romance, Sci-Fi, Thriller, War, Western
    def _load_year_and_genre(self, filename):
        mapping_dic = {}
        with open(self.dirname + 'data/' + filename, 'r') as f:
            for line in f:
                words = map(int, line.split())
                mapping_dic[words[0]] = words[1:]
        return mapping_dic


    def _load_ratings_pop(self,filename):
        # compactID =  avgRating, numberRating, rank
        popularity = {}
        with open(self.dirname + 'data/' + filename, 'r') as f:
            for line in f:
                words = map(float, line.split())
                popularity[int(words[0])] = words[1:]
